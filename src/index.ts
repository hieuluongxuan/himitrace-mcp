...
const RESOURCES = {
  ...
  "himitrace://contract": {
    ...
    read: () => {
      if (!chain.connected) return "Blockchain not connected";
  ...
  server.setRequestHandler(ListResourcesRequestSchema, async () => {
    return {
      resources: Object.keys(RESOURCES).map((uri) => {
        const res = RESOURCES[uri as keyof typeof RESOURCES];
        return {
          uri,
          name: res.name,
  ...
  if (isSse) {
    ...
    app.all("/sse", async (req, res) => {
      ...
      try {
        ...
        res.on("close", () => {
          console.log("[Streamable HTTP] Connection closed, cleaning up");
        ...
        console.error("[Streamable HTTP Handler Error]", err);
        if (!res.headersSent) {
          res.status(500).json({
            jsonrpc: "2.0",
            error: {
